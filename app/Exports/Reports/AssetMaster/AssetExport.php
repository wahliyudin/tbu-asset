...
class AssetExport implements FromCollection, WithMapping, WithTitle, ShouldAutoSize, WithHeadings
{
    ...
    public function map($row): array
    {
        return [
            ...
            $row->asset_unit?->type,
            $row->asset_unit?->seri,
            $row->asset_unit?->class,
            $row->asset_unit?->brand,
            $row->asset_unit?->serial_number,
            $row->asset_unit?->spesification,
            $row->asset_unit?->tahun_pembuatan,
            $row->asset_unit?->kelengkapan_tambahan,
            $row->sub_cluster?->cluster?->category?->name,
            $row->sub_cluster?->cluster?->name,
            $row->sub_cluster?->name,
